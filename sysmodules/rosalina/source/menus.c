...
static volatile bool g_brightnessOverclockEnabled = false;
...
static void brightnessThreadMain(void)
{
    ...
    while (g_brightnessOverclockEnabled) {
        setLuminance(400, true);
        setLuminance(400, false);
        svcSleepThread(7000000LL); // 7ms when active
    }
    
    // Thread is exiting — reset to default brightness and clean up
    setLuminance(172, true);   // top screen
    setLuminance(172, false);  // bottom screen
    g_brightnessThreadRunning = false;
    svcCloseHandle(g_brightnessThread);
    g_brightnessThread = 0;
    svcExitThread();
}
...
void RosalinaMenu_OverclockBrightness(void)
{
    ...
    if (g_brightnessOverclockEnabled) {
        ...
        // Spawn thread if not already running
        ...
        // Thread will see g_brightnessOverclockEnabled == false, reset brightness, and exit on its own
    }
}
